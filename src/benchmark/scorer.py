...
_VAGUE_PHRASES = [
    "various", "general", "miscellaneous", "etc", "and so on",
    "handle things", "do stuff", "take care of", "deal with",
    "as needed", "if necessary", "when required", "as appropriate",
    "ensure quality", "improve processes", "manage tasks",
    "perform duties", "assigned duties", "daily activities",
]
...
def score_tasks(parsed_tasks: list[dict], ground_truth: dict) -> dict:
    ...
    if not parsed_tasks:
        return {"coverage": 0, "relevance": 0, "specificity": 0, "non_redundancy": 0, "score": 0}
    ...
    generic_count = sum(1 for t in parsed_tasks if any(g in t.get("title", "").lower() for g in generic))
    relevance = max(0, 1.0 - generic_count * 0.25)

    titles = [t.get("title", "") for t in parsed_tasks]
    all_step_descs = [s.get("description", "") if isinstance(s, dict) else str(s)
                      for t in parsed_tasks for s in t.get("steps", [])]
    ...
    score = round(0.30 * coverage + 0.25 * relevance + 0.25 * specificity + 0.20 * non_redundancy, 3)
    return {"coverage": round(coverage, 3), "relevance": round(relevance, 3),
            "specificity": round(specificity, 3), "non_redundancy": round(non_redundancy, 3),
            "score": score}
...
def score_steps(parsed_tasks: list[dict], ground_truth: dict) -> dict:
    ...
    if not all_steps:
        return {"atomicity": 0, "logical_ordering": 0, "actionability": 0, "specificity": 0, "score": 0}

    gt_keywords = ground_truth.get("steps_keywords", [])
    all_text = " ".join(all_steps).lower()

    compound_markers = [" and then ", " followed by ", " while also ", "; "]
    compound_count = sum(1 for s in all_steps for m in compound_markers if m in s.lower())
    atomicity = max(0, 1.0 - compound_count * 0.15)

    overlaps = []
    for t in parsed_tasks:
        steps = [s.get("description", "") if isinstance(s, dict) else str(s) for s in t.get("steps", [])]
    ...
    score = round(0.25 * atomicity + 0.25 * logical_ordering + 0.25 * actionability + 0.25 * specificity, 3)
    return {"atomicity": round(atomicity, 3), "logical_ordering": round(logical_ordering, 3),
            "actionability": round(actionability, 3), "specificity": round(specificity, 3),
            "score": score}
...
def score_problems(parsed_analyses: list[dict], ground_truth: dict) -> dict:
    ...
    causal_words = ["because", "due to", "causes", "leads to", "results in", "prone to", "risk of", "which means"]
    ...
    return {"specificity": round(specificity, 3), "realism": round(realism, 3),
            "causality": round(causality, 3), "score": score}
...
def score_solutions(parsed_solutions: list[dict], ground_truth: dict, bad_patterns: list[str]) -> dict:
    if not parsed_solutions:
        return {"correctness": 0, "specificity": 0, "diversity": 0, "feasibility": 0, "novelty": 0, "score": 0}
    ...
    with_rationale = sum(1 for s in parsed_solutions if len(s.get("rationale", "")) > 10)
    ...
    score = round(0.25 * correctness + 0.25 * specificity + 0.20 * diversity + 0.15 * feasibility + 0.15 * novelty, 3)
    return {"correctness": round(correctness, 3), "specificity": round(specificity, 3),
            "diversity": round(diversity, 3), "feasibility": round(feasibility, 3),
            "novelty": round(novelty, 3), "score": score}
...
def score_evaluations(parsed_evals: list[dict]) -> dict:
    if not parsed_evals:
        return {"variance": 0, "justification": 0, "specificity": 0, "usefulness": 0, "score": 0}
    ...
    complete = sum(1 for e in parsed_evals
    ...
    for e in parsed_evals:
        unique_combos.add((e.get("feasibility", 0), e.get("impact", 0), e.get("complexity", 0)))
    specificity = len(unique_combos) / max(len(parsed_evals), 1)

    high_quality = sum(1 for e in parsed_evals if e.get("feasibility", 0) >= 4 and e.get("impact", 0) >= 4)
    usefulness = min(1.0, high_quality / 2)

    score = round(0.25 * variance + 0.30 * justification + 0.20 * specificity + 0.25 * usefulness, 3)
    return {"variance": round(variance, 3), "justification": round(justification, 3),
            "specificity": round(specificity, 3), "usefulness": round(usefulness, 3),
            "score": score}
...
def score_system(parsed_tasks, parsed_analyses, parsed_solutions, parsed_evals, bad_patterns) -> dict:
    all_text = json.dumps(
        (parsed_tasks or []) + (parsed_analyses or []) + (parsed_solutions or []) + (parsed_evals or [])
    ).lower()

    consistency = 1.0
    if parsed_solutions and parsed_analyses:
        sol_problems = {s.get("problem", "").lower()[:30] for s in parsed_solutions}
        ana_problems = {a.get("problem", "").lower()[:30] for a in parsed_analyses if a.get("problem")}
        if ana_problems:
    ...
    stages_present = sum([bool(parsed_tasks), bool(parsed_analyses), bool(parsed_solutions), bool(parsed_evals)])
    practicality = stages_present / 4
...
def score_pipeline(run: dict, sample: dict) -> dict:
    ...
    s1 = stages[0].get("parsed", []) if len(stages) > 0 and stages[0].get("valid") else []
    s2 = stages[1].get("parsed", []) if len(stages) > 1 and stages[1].get("valid") else []
    s3 = stages[2].get("parsed", []) if len(stages) > 2 and stages[2].get("valid") else []
    s4 = stages[3].get("parsed", []) if len(stages) > 3 and stages[3].get("valid") else []

    task_scores = score_tasks(s1, gt)
    step_scores = score_steps(s1, gt)
    problem_scores = score_problems(s2, gt)
    solution_scores = score_solutions(s3, gt, bad)
    eval_scores = score_evaluations(s4)
    system_scores = score_system(s1, s2, s3, s4, bad)
    ...
    sys_avg = (system_scores["consistency"] + system_scores["redundancy"]
               + system_scores["constraint_adherence"] + system_scores["practicality"]) / 4
